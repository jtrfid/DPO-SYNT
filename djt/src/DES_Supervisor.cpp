...
#include <sstream>
#include "../getopt/include/getopt.h"
...
// /f 强制删除只读文件 /s 从所有子目录删除指定文件  /q 安静模式.删除全局通配符时，不要求确认。
const char* const INITIAL_CLEAN_UP = "del /f/s/q ..\\results\\*.*"; 

//rmdir /S  除目录本身外，还将删除指定目录下的所有子目录和文件。用于删除目录树。 /Q 安静模式
//const char* const INITIAL_CLEAN_UP = "rmdir /s/q ..\\results";
